    ...
    execute: async (interaction: ChatInputCommandInteraction) => {
        ...
        if (targetUser) {
            ...
            if (!record || record.offenseCount === 0) {
                ...
                embed.addFields(
                    { name: 'Offenses (24h)', value: `${record.offenseCount}`, inline: true },
                    { name: 'Last Incident', value: time(Math.floor(record.lastOffenseTime / 1000), TimestampStyles.RelativeTime), inline: true }
                );
            }
        } else {
            ...
            if (sorted.length === 0) {
                ...
                const description = sorted.map(([id, record]) => {
